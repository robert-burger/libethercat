...
typedef struct ec {
    ...
    pool_entry_t mbx_mp_recv_free_entries[LEC_MAX_MBX_ENTRIES];  //!< \brief static buffers for
                                                                 //!< mailbox receive pool.
    pool_entry_t mbx_mp_send_free_entries[LEC_MAX_MBX_ENTRIES];  //!< \brief static buffers for
                                                                 //!< mailbox send pool.
    ...
    int consecutive_max_miss;  //!< max missed counter for receive frames before falling back to
                               //!< init
...
#define ec_group_will_be_sent(pec, group)                                                      \
    (int)((((pec)->pd_groups[(group)].divisor_cnt + 1) % (pec)->pd_groups[(group)].divisor) == \
          0)  //!< \brief Group datagram will be send on next time step.
#define ec_group_was_sent(pec, group) \
    (int)((pec)->pd_groups[(group)].divisor_cnt == 0)  //!< \brief Group datagram was sent.
...
#define check_ec_bwr(...) \
    check_ret(            \
        ec_bwr,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_brd(...) \
    check_ret(            \
        ec_brd,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
...
#define check_ec_apwr(...) \
    check_ret(             \
        ec_apwr,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_aprd(...) \
    check_ret(             \
        ec_aprd,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_aprw(...) \
    check_ret(             \
        ec_aprw,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.

#define check_ec_fpwr(...) \
    check_ret(             \
        ec_fpwr,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_fprd(...) \
    check_ret(             \
        ec_fprd,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_fprw(...) \
    check_ret(             \
        ec_fprw,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
#define check_ec_frmw(...) \
    check_ret(             \
        ec_frmw,           \
        __VA_ARGS__)  //!< \brief Macro that checks return value and prints message in error case.
