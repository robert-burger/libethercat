...
typedef struct ec_coe {
    ...
    ec_coe_emergency_message_t
        emergencies[LEC_MAX_COE_EMERGENCIES];  //!< \brief emergency message ring buffer.
 ...
 * \retval EC_ERROR_MAILBOX_BUFFER_TOO_SMALL    Provided buffer was too small, returning size in \p
 * len. \retval EC_ERROR_MAILBOX_ABORT               Got SDO abort request. \retval
 * EC_ERROR_MAILBOX_OUT_OF_SEND_BUFFERS No more free send buffer available. \retval
 * EC_ERROR_MAILBOX_NOT_SUPPORTED_COE   No CoE support on slave's mailbox. \retval
 * EC_ERROR_UNAVAILABLE                 Locking mailbox failed for some reason. \retval
