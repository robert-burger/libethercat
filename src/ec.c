...
// forward declaration
static void default_log_func(ec_t* pec, int lvl, const osal_char_t* format, ...)
...
static int64_t signed64_diff(osal_uint64_t a, osal_uint64_t b) {
    ...
    if (abs_diff > (osal_uint64_t)INT64_MAX) {
        ...
        if (tmp_a < tmp_b) {
            abs_diff = (tmp_b - tmp_a);
        }
    }
    return (tmp_a > tmp_b) ? (int64_t)abs_diff : -(int64_t)abs_diff;
}

//! calculate signed difference of 32-bit unsigned int's
/*!
 * \param[in]   a       Minuend.
 * \param[in]   b       Subtrahend.
 *
...
static int32_t signed32_diff(osal_uint32_t a, osal_uint32_t b) {
    ...
    if (abs_diff > (osal_uint32_t)INT_MAX) {
        if (a > (osal_uint32_t)INT_MAX) {
            tmp_a = UINT_MAX - a;
            tmp_b = b;
        } else if (b > (osal_uint32_t)INT_MAX) {
            tmp_a = a;
            tmp_b = UINT_MAX - b;
        } else {
        }
    ...
    return (tmp_a > tmp_b) ? (int32_t)abs_diff : -(int32_t)abs_diff;
}
...
static void ec_create_logical_mapping_overlapping(ec_t* pec, osal_uint32_t group) {
    ...
    ec_log(10, "CREATE_LOGICAL_MAPPING",
           "group %2d: pd out 0x%08X "
           "%3" PRIu64 " bytes, in 0x%08" PRIx64 " %3" PRIu64 " bytes, lrw window %3" PRIu64
           " bytes, expected wkc %d\n",
           group, pd->log, pd->pdout_len, pd->log + pd->pdout_len, pd->pdin_len, pd->pd_lrw_len,
           pd->wkc_expected_lrw);
...
static void ec_create_logical_mapping(ec_t* pec, osal_uint32_t group) {
    ...
    ec_log(10, "CREATE_LOGICAL_MAPPING",
           "group %2" PRIu32 ": pd out 0x%08" PRIx32 "%3" PRIu64 " bytes, in 0x%08" PRIx64
           " %3" PRIu64 " bytes\n",
           group, pd->log, pd->pdout_len, pd->log + pd->pdout_len, pd->pdin_len);
        ...
        for (osal_uint32_t slave = 0u; slave < pec->slave_cnt; ++slave) {
            if (pec->slaves[slave].assigned_pd_group != -1) {
                ec_log(100, get_state_string(state),
                       "prepare state transition for slave %" PRIu32 "\n", slave);
                int ret = ec_slave_prepare_state_transition(pec, slave, state);
                if (ret != EC_OK) {
                    ec_log(1, get_state_string(state),
                           "ec_slave_prepare_state_transition failed with %d\n", ret);
                }
    ...
    if (pec->threaded_startup != 0) {
        for (osal_uint32_t slave = 0u; slave < pec->slave_cnt; ++slave) {
            if ((with_group == 0u) || (pec->slaves[slave].assigned_pd_group != -1)) {
                ...
                attr.affinity = 0xFF;
                (void)snprintf(&attr.task_name[0], TASK_NAME_LEN, "ecat.worker%" PRIu32, slave);
                osal_task_create(&(pec->slaves[slave].worker_tid), &attr, set_state_wrapper,
                                 &(pec->slaves[slave].worker_arg));
            }
        }
...
static void ec_scan(ec_t* pec) {
    ...
    if (ret != EC_OK) {
        ...
        for (i = 0; i < pec->slave_cnt; ++i) {
            ...
            int local_ret =
                ec_aprd(pec, auto_inc, EC_REG_TYPE, (osal_uint8_t*)&val, sizeof(val), &wkc);
            ...
            if (local_ret == EC_OK) {
                ...
                local_ret = ec_apwr(pec, auto_inc, EC_REG_STADR, (osal_uint8_t*)&fixed,
                                    sizeof(fixed), &wkc);
        ...
        for (osal_uint16_t slave = 0; slave < pec->slave_cnt; ++slave) {
            ...
            int local_ret =
                ec_fprd(pec, slv->fixed_address, EC_REG_DLSTAT, &dlstat, sizeof(dlstat), &wkc);
            ...
            if (local_ret == EC_OK) {
                    ...
                    for (int tmp_slave = (int)slave - 1; tmp_slave >= 0; --tmp_slave) {
                        switch (pec->slaves[tmp_slave].link_cnt) {
                            case 1u:  // endpoint, skip this chain (presumable behind a bus coupler)
                                topology--;
                                break;
                            case 3u:  // split, add this, found a bus splitter (presumable a bus
                                      // coupler)
                                topology++;
                                break;
                            case 4u:  // cross,
                        ...
                        if (((topology >= 0) && (pec->slaves[tmp_slave].link_cnt > 1u)) ||
                            (tmp_slave == 0)) {
                ...
                slv->ptype = 0;
                local_ret = ec_fprd(pec, slv->fixed_address, EC_REG_PORTDES, &slv->ptype,
                ...
                for (int port = 0; port < 4; ++port) {
                    ...
                    switch (port_val) {
                        default:
                            break;
                        case 1:
                            ec_log(100, "MASTER_SCAN",
                                   "slave %2d: port %d is not configured (SII EEPROM)\n", slave,
                                   port);
                            break;
                        case 2:
...
int ec_set_state(ec_t* pec, ec_state_t state) {
    ...
    ec_state_transition_t transition =
        ((pec->master_state & EC_STATE_MASK) << 8) | (state & EC_STATE_MASK);
    ...
    switch (transition) {
        ...
        case UNKNOWN_2_OP:
            ...
            if (pec->user_cb_state_transition != NULL) {
                pec->user_cb_state_transition(pec->user_cb_state_transition_arg, pec, EC_STATE_INIT,
                                              OSAL_TRUE);
            }
        ...
        case PREOP_2_PREOP:
            ...
            if (pec->user_cb_state_transition != NULL) {
                pec->user_cb_state_transition(pec->user_cb_state_transition_arg, pec,
                                              EC_STATE_PREOP, OSAL_TRUE);
            }
            ...
            for (osal_uint16_t group = 0u; group < pec->pd_group_cnt; ++group) {
                for (osal_uint16_t slave = 0u; slave < pec->slave_cnt; ++slave) {
                    if ((pec->slaves[slave].assigned_pd_group == group) &&
                        ((pec->slaves[slave].features & EC_REG_ESCSUP__NOT_SUPP_LRW) ==
                         EC_REG_ESCSUP__NOT_SUPP_LRW)) {
                ...
                if (pec->pd_groups[group].overlapping != 0) {
                    ec_log(10, get_state_string(pec->master_state),
                           "group %2d: using LRW, "
        ...
        case OP_2_OP: {
            // ====> switch to OP stuff
            if (pec->user_cb_state_transition != NULL) {
                pec->user_cb_state_transition(pec->user_cb_state_transition_arg, pec, EC_STATE_OP,
                                              OSAL_TRUE);
            }
...
int ec_open(ec_t* pec, struct hw_common* phw, int eeprom_log) {
    ...
    (void)pool_open(&pec->mbx_message_pool_recv_free, LEC_MAX_MBX_ENTRIES,
                    &pec->mbx_mp_recv_free_entries[0]);
    (void)pool_open(&pec->mbx_message_pool_send_free, LEC_MAX_MBX_ENTRIES,
                    &pec->mbx_mp_send_free_entries[0]);
...
void ec_decode_datagram_to_string(ec_datagram_t* p_dg, char* out, ssize_t out_len) {
    uint16_t adp = p_dg->adr & 0xFFFFu, ado = (p_dg->adr & 0xFFFF0000u) >> 16u;
    ...
    switch (p_dg->cmd) {
        ...
        case EC_CMD_BWR:
        case EC_CMD_BRW:
            snprintf(out, out_len, "cmd %s, register 0x%04X", ec_datagram_cmd_to_string(p_dg->cmd),
                     ado);
            break;
        case EC_CMD_LRD:
        case EC_CMD_LWR:
 ...
 * \return 0 on succes, otherwise error code
 */
int ec_transceive(ec_t* pec, osal_uint8_t cmd, osal_uint32_t adr, osal_uint8_t* data,
                  osal_size_t datalen, osal_uint16_t* wkc) {
    assert(pec != NULL);
    assert(data != NULL);
    ...
    if (ec_index_get(&pec->idx_q, &p_idx) != EC_OK) {
            ...
            if (local_ret != OSAL_OK) {
                if (local_ret == OSAL_ERR_TIMEOUT) {
                    ...
                    ec_log(1, "MASTER_TRANSCEIVE",
                           "osal_binary_semaphore_wait returned: %d, cmd 0x%X, adr 0x%X\n",
                           local_ret, cmd, adr);
                }
        ...
        if (to_transceive_expired == OSAL_ERR_TIMEOUT) {
            ec_log(1, "MASTER_TRANSCEIVE",
                   ...
                   "This should usually not happen on an EtherCAT fieldbus because the sent frames "
                   "must always return to the master.\n"
                   "There's either something wrong with your configuration or there is a hardware "
                   "issue on your bus topology!\n"
                   "Lost datagram during transceive though it was sent multiple times! Check your "
                   "configuration!\n",
                   cmd, adr);
        ...
        if (do_check_group == OSAL_TRUE) {
            if ((pd->wkc_mismatch_cnt_lwr++ % 1000) == 0) {
                ec_log(1, "MASTER_RECV_PD_LWR",
                       ...
                       pd->group, wkc, wkc_expected, pec->slave_cnt, pd->wkc_mismatch_cnt_lwr);
            }
...
static void cb_process_data_group(struct ec* pec, pool_entry_t* p_entry, ec_datagram_t* p_dg) {
    ...
    if (pd->pdin_len > 0 && (pd->skip_pd_on_wkc_mismatch ? !wkc_mismatch : OSAL_TRUE)) {
        if ((pd->use_lrw != 0) || (pd->overlapping)) {
            ...
            (void)memcpy(&pd->pd[pd->pdout_len], &ec_datagram_payload(p_dg)[pd->pdout_len],
                         pd->pdin_len);
        }
    }
    ...
    if (((pec->master_state == EC_STATE_SAFEOP) || (pec->master_state == EC_STATE_OP)) &&
        ...
        if ((pd->wkc_mismatch_cnt_lrw++ % 1000) == 0) {
            ec_log(1, "MASTER_RECV_PD_GROUP",
                   "group %2" PRIu32
                   ": working counter mismatch got %u, "
                   "expected %u, slave_cnt %d, mismatch_cnt %d\n",
                   pd->group, wkc, wkc_expected, pec->slave_cnt, pd->wkc_mismatch_cnt_lrw);
        }
    ...
    if (wkc == pd->wkc_expected_mbx_state) {
        ...
        if (((pec->master_state == EC_STATE_SAFEOP) || (pec->master_state == EC_STATE_OP)) &&
            ...
            ec_log(1, "MASTER_RECV_MBX_STATE",
                   ...
                   ": working counter mismatch got %u, "
                   "expected %u, slave_cnt %d, mismatch_cnt %d\n",
                   pd->group, wkc, pd->wkc_expected_mbx_state, pec->slave_cnt,
                   pd->wkc_mismatch_cnt_mbx_state);
        }
    }
...
static void cb_distributed_clocks(struct ec* pec, pool_entry_t* p_entry, ec_datagram_t* p_dg) {
    ...
    if (wkc != 0u) {
        ...
        int64_t tx_time_to_dc_master =
            (pec->dc.packet_duration - pec->slaves[pec->dc.next].dc.t_delay_with_childs) * -0.4;

        if (pec->dc.have_64bit == 0) {  // we only have 32-bit dc ...
            // get clock difference
            pec->dc.act_diff = signed32_diff((pec->dc.rtc_time % UINT_MAX + tx_time_to_dc_master),
                                             pec->dc.dc_time);
        } else {
            // get clock difference
            pec->dc.act_diff = signed64_diff((pec->dc.rtc_time % UINT64_MAX + tx_time_to_dc_master),
                                             pec->dc.dc_time);
        }
        ...
        if (pec->dc.mode == dc_mode_ref_clock) {
            ...
            if (ec_index_get(&pec->idx_q, &p_idx_sto) != EC_OK) {
                ...
                p_dg2->irq = 0;
                (void)memcpy(ec_datagram_payload(p_dg2), (osal_uint8_t*)&pec->dc.dc_sto,
                             sizeof(pec->dc.dc_sto));
        ...
        if (ret == EC_OK) {
            ...
            if (pec->dc.mode == dc_mode_ref_clock) {
                ...
                if (pec->dc.mode == dc_mode_master_as_ref_clock) {
                    ...
                    int64_t tx_time_to_dc_master =
                        (pec->dc.packet_duration -
                         pec->slaves[pec->dc.next].dc.t_delay_with_childs) *
                        -0.4;
                    pec->dc.rtc_time += tx_time_to_dc_master;

                    (void)memcpy((osal_uint8_t*)ec_datagram_payload(p_dg),
                                 (osal_uint8_t*)&pec->dc.rtc_time, sizeof(pec->dc.rtc_time));
                }
            }
...
static void cb_brd_ec_state(struct ec* pec, pool_entry_t* p_entry, ec_datagram_t* p_dg) {
    ...
    if (!pec->state_transition_pending && (al_status != pec->master_state)) {
        ...
        if ((ec_state_mismatch_cnt++ % 1000) == 0) {
            ec_log(1, "MASTER_RECV_BRD_STATE",
                   "al status mismatch, got 0x%X, master state is 0x%X\n", al_status,
                   pec->master_state);
