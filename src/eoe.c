...
static void eoe_debug_print(ec_t* pec, const osal_char_t* ctx, const osal_char_t* msg,
                            osal_uint8_t* frame, osal_size_t frame_len) {
...
int ec_eoe_set_ip_parameter(ec_t* pec, osal_uint16_t slave, osal_uint8_t* mac,
    ...
    if (ec_mbx_check(pec, slave, EC_EEPROM_MBX_EOE) != EC_OK) {
        ...
        ec_eoe_set_ip_parameter_request_t* write_buf =
            (ec_eoe_set_ip_parameter_request_t*)(p_entry->data);
    ...
    if ((parameter) != NULL) {                                         \
        ...
        if (p_entry != NULL) {
            ...
            ec_eoe_set_ip_parameter_response_t* read_buf =
                (ec_eoe_set_ip_parameter_response_t*)(p_entry->data);
...
int ec_eoe_send_frame(ec_t* pec, osal_uint16_t slave, osal_uint8_t* frame, osal_size_t frame_len) {
    ...
    if (ec_mbx_check(pec, slave, EC_EEPROM_MBX_EOE) != EC_OK) {
        ...
        osal_size_t max_frag_len =
            (slv->sm[MAILBOX_WRITE].len - sizeof(ec_mbx_header_t) - sizeof(ec_eoe_header_t));
        ...
        do {
            ...
            if (frag_len < max_frag_len) {
                write_buf->eoe_hdr.last_fragment = 0x01u;
            } else {
                write_buf->eoe_hdr.last_fragment = 0x00u;
            }

            if (frag_number == 0) {
                write_buf->eoe_hdr.complete_size = (frame_len + 31u) >> 5u;
            } else {
                write_buf->eoe_hdr.complete_size = (frame_offset) >> 5u;
            }
...
int ec_eoe_process_recv(ec_t* pec, osal_uint16_t slave) {
    ...
    if (pool_get(&slv->mbx.eoe.recv_pool, &p_entry, NULL) !=
        EC_OK) {  // get first received EoE frame, should be start of ethernet frame
        ...
        // no usable Ethernet frame buffer available, nobody has cared for last received Ethernet
        // frames so far.
        ...
        if (read_buf->eoe_hdr.fragment_number != 0u) {
            ...
            if (!read_buf->eoe_hdr.last_fragment) {
                ...
                while (p_entry != NULL) {
                    ...
                    (void)memcpy(&(eth_frame->frame_data[frame_offset]), &(read_buf->data[0]),
                                 frag_len);
                    frame_offset += frag_len;
                ...
                if (pec->tun_fd > 0) {
                    eoe_debug_print(pec, "EOE_RECV", "recv eth frame", eth_frame->frame_data,
                                    frame_offset);
    ...
    while (pec->tun_running == 1) {
        ...
        if (ret < 0) {
            ...
            if (FD_ISSET(pec->tun_fd, &rd_set) != 0) {
                ...
                if (rd > 0) {
                    // simple switch here
                    eoe_debug_print(pec, "EOE_TUN_HANDLER", "got eth frame", tmp_frame.frame_data,
                                    rd);

                    static const osal_uint8_t broadcast_mac[] = {0xFF, 0xFF, 0xFF,
                                                                 0xFF, 0xFF, 0xFF};
                    ...
                    for (osal_uint16_t slave = 0; slave < pec->slave_cnt; ++slave) {
                        ec_slave_ptr(slv, pec, slave);
                        if (!slv->eoe.use_eoe || (slv->act_state == EC_STATE_INIT) ||
