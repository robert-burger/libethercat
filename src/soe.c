...
void ec_soe_enqueue(ec_t* pec, osal_uint16_t slave, pool_entry_t* p_entry) {
    ...
    if (soe_hdr->op_code == EC_SOE_NOTIFICATION) {
        ...
        local_ret = snprintf(soe_log_buf, SOE_LOG_BUF_SIZE,
                             "SoE Notification: opcode %d, incomplete %d, error %d, "
                             "atn %d, elements %d, idn %d",
                             soe_hdr->op_code, soe_hdr->incomplete, soe_hdr->error, soe_hdr->atn,
                             soe_hdr->elements, soe_hdr->idn_fragments_left);
        if (local_ret >= 0) {
            osal_uint32_t soe_log_pos = (osal_uint32_t)local_ret;

            if ((mbx_hdr->length - sizeof(ec_soe_header_t)) > 0u) {
                local_ret = snprintf(&soe_log_buf[soe_log_pos], SOE_LOG_BUF_SIZE - soe_log_pos,
                                     ", payload: ");

                if (local_ret >= 0) {
                    soe_log_pos += (osal_uint32_t)local_ret;

                    osal_uint8_t* payload =
                        (osal_uint8_t*)(&p_entry->data[sizeof(ec_mbx_header_t) +
                                                       sizeof(ec_soe_header_t)]);

                    for (osal_uint32_t i = 0; i < (mbx_hdr->length - sizeof(ec_soe_header_t));
                         ++i) {
                        local_ret = snprintf(&soe_log_buf[soe_log_pos],
                                             SOE_LOG_BUF_SIZE - soe_log_pos, "%02X", payload[i]);
                        if (local_ret < 0) {
                            break;
                        }
 ...
 * \link ec_soe_read \endlink is a synchronous call and will block until it's either finished or
 * aborted.
 *
 * \param[in] pec       Pointer to ethercat master structure,
 *                      which you got from \link ec_open \endlink.
    ...
    if (ec_mbx_check(pec, slave, EC_EEPROM_MBX_SOE) != EC_OK) {
        ...
        while (p_entry != NULL) {
            ...
            if (read_buf->soe_hdr.op_code != EC_SOE_READ_RES) {
                ec_log(5, "SOE_READ",
                       "got unexpected response: opcode %d, incomplete %d, error %d, "
                       "atn %d, elements %d, idn %d\n",
                       read_buf->soe_hdr.op_code, read_buf->soe_hdr.incomplete,
                       read_buf->soe_hdr.error, read_buf->soe_hdr.atn, read_buf->soe_hdr.elements,
                       read_buf->soe_hdr.idn_fragments_left);
                ec_mbx_return_free_recv_buffer(pec, slave, p_entry);
...
int ec_soe_write(ec_t* pec, osal_uint16_t slave, osal_uint8_t atn, osal_uint16_t idn,
    ...
    if (ec_mbx_check(pec, slave, EC_EEPROM_MBX_SOE) != EC_OK) {
        ...
        osal_size_t mbx_len =
            slv->sm[MAILBOX_WRITE].len - sizeof(ec_mbx_header_t) - sizeof(ec_soe_header_t);

        ec_log(100, "SOE_WRITE",
               "slave %d, atn %d, idn %d, elements %d, buf %p, "
        ...
        for (osal_uint32_t i = 0; i < len; ++i) {
            int local_ret =
                snprintf(&soe_log_buf[soe_log_pos], SOE_LOG_BUF_SIZE - soe_log_pos, "%02X", buf[i]);
            if (local_ret >= 0) {
                soe_log_pos += (osal_uint32_t)local_ret;
            } else {
        ...
        while ((left_len != 0u) && (ret != EC_ERROR_MAILBOX_OUT_OF_WRITE_BUFFERS)) {
            if (ec_mbx_get_free_send_buffer(pec, slave, &p_entry, NULL) != EC_OK) {
                ...
                ec_log(100, "SOE_WRITE",
                       "slave %d, atn %d, idn %d, elements %d: sending fragment len %" PRIu64
                       " (left %" PRIu64 ")\n",
                ...
                while (p_entry != NULL) {
                    ...
                    ec_log(100, "SOE_WRITE",
                           "got response: opcode %d, incomplete %d, error %d, "
                           "atn %d, elements %d, idn %d\n",
                           read_buf->soe_hdr.op_code, read_buf->soe_hdr.incomplete,
                           read_buf->soe_hdr.error, read_buf->soe_hdr.atn,
                           read_buf->soe_hdr.elements, read_buf->soe_hdr.idn_fragments_left);
    ...
    *bitsize = 16u;  // control and status word are always present
    ...
    if (ret == EC_OK) {
        ...
        for (i = 0u; i < (idn_len[0] / 2u); ++i) {
            ...
            if (ec_soe_read(pec, slave, atn, sub_idn, &elements, buf, &sub_idn_attr_size) !=
                EC_OK) {
                continue;
            }
