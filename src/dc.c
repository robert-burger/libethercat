...
const osal_uint8_t forward_port_order[4][4] = {
    {3, 1, 2, 0}, {2, 0, 3, 1}, {0, 3, 1, 2}, {1, 2, 0, 3}};
const osal_uint8_t reverse_port_order[4][4] = {
 ...
 */
int ec_dc_sync(ec_t* pec, osal_uint16_t slave, osal_uint8_t active, osal_uint32_t cycle_time_0,
        ...
        if ((active != 0u) && (pec->dc.dc_time == 0u)) {
            ...
            do {
                if (osal_timer_expired(&dc_time_to) == OSAL_ERR_TIMEOUT) {
                    ec_log(1, "DC_SYNC",
                           "slave %2d: ERROR calculating start time because there's no cyclic "
    ...
    if (ret == EC_OK) {
        ...
        check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCSYNCACT, &dc_active, sizeof(dc_active),
                      &wkc);
        ...
        dc_start = ((pec->dc.dc_time / pec->main_cycle_interval) * pec->main_cycle_interval) +
                   cycle_shift + ONE_SEC;
        ...
        check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCCYCLE0, &cycle_time_0, sizeof(cycle_time_0),
                      &wkc);
        check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCCYCLE1, &cycle_time_1, sizeof(cycle_time_1),
                      &wkc);
        ...
        if (dc_active != 0u) {
            ec_log(10, "DC_SYNC",
                   "slave %2d: dc_systime %" PRIu64 ".%09" PRIu64
                   " s, dc_start "
                   "%" PRId64 ".%09" PRIu64 " s, slv dc_time %" PRId64 ".%09" PRIu64
                   " s, pdelay %d\n",
                   slave, pec->dc.dc_time / 1000000000, pec->dc.dc_time % 1000000000,
                   dc_start / 1000000000, dc_start % 1000000000, dc_time / 1000000000,
                   dc_time % 1000000000, slv->pdelay);
            ec_log(10, "DC_SYNC",
                   "slave %2d: cycletime_0 %" PRIu32 ", cycletime_1 %" PRIu32
                   ", "
                   "dc_active %d\n",
    ...
    for (osal_uint16_t slave = 0; slave < pec->slave_cnt; slave++) {
        ...
        if ((slv->dc.use_dc == 0) || ((slv->features & EC_REG_ESCSUP__DC_SUPP) == 0u)) {
            ec_log(100, "DC_CONFIG", "slave %2d: not using DC (use %d, supported %X)\n", slave,
                   slv->dc.use_dc, slv->features & EC_REG_ESCSUP__DC_SUPP);
        ...
        check_ec_fprd(pec, slv->fixed_address, EC_REG_DCTIME0, &slv->dc.receive_times[0],
                      4 * sizeof(slv->dc.receive_times[0]), &wkc);
        slv->entry_port = -1;
        for (i = 0u; i < 4u; ++i) {
            if ((slv->active_ports & (1u << i)) != 0u) {
                if ((slv->entry_port == -1) ||
                    (slv->dc.receive_times[i] < slv->dc.receive_times[slv->entry_port])) {
                    // port with smallest value is our entry port (which should be port 0 in most
                    // cases, see ET1100, Section 1, 3.7)
                    slv->entry_port = i;
                }

                ec_log(100, "DC_CONFIG",
                       "slave %2d: receive time port %" PRIu32 " is %" PRIu32 "\n", slave, i,
                       slv->dc.receive_times[i]);
            }
        }

        if (slv->entry_port != 0) {
            // we have weird order, processing unit sits behind port 0, so processing is done in
            // wrong order
            ec_log(5, "DC_CONFIG",
                   "slave %2d: entry port is not the first port, check wiring order!\n", slave);
            ...
            if (slv->type == 0x1200) {
                ...
                dc_time0 = 0;
                check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCTIME0, &dc_time0, sizeof(dc_time0),
                              &wkc);
                check_ec_fprd(pec, slv->fixed_address, EC_REG_DCTIME0, &slv->dc.receive_times[0],
                              4 * sizeof(slv->dc.receive_times[0]), &wkc);
                for (i = 0u; i < 4u; ++i) {
        ...
        ec_log(100, "DC_CONFIG", "slave %2d: available_ports 0x%X, entry_port %d\n", slave,
               slv->dc.available_ports, slv->entry_port);
        ...
        check_ec_fprd(pec, slv->fixed_address, EC_REG_DCSPEEDCNT, &speed_counter_start,
                      sizeof(speed_counter_start), &wkc);
        check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCSPEEDCNT, &speed_counter_start,
                      sizeof(speed_counter_start), &wkc);
        ...
        if (pec->dc.master_address == slv->fixed_address) {
            ...
            pec->dc.rtc_sto =
                (osal_timer_gettime_nsec() / pec->main_cycle_interval) * pec->main_cycle_interval;
        ...
        for (parent = slv->parent; parent >= 0; parent = pec->slaves[parent].parent) {
            ...
            if ((pec->slaves[parent].dc.use_dc) &&
                (pec->slaves[parent].features & EC_REG_ESCSUP__DC_SUPP)) {
                break;
            }
        }
        ...
        slv->dc.t_delay_childs =
            (osal_int32_t)times_slave[last_connected_port] - times_slave[slv->entry_port];
        ...
        if (parent >= 0) {
            ...
            slv->port_on_parent =
                get_and_consume_next_available_port(slv_parent, slv_parent->entry_port);
            parent_previous_port = get_previous_active_port(slv_parent, slv->port_on_parent);

            slv->dc.t_delay_with_childs = abs((osal_int32_t)times_parent[slv->port_on_parent] -
                                              (osal_int32_t)times_parent[parent_previous_port]);
            slv->dc.t_delay_slave =
                abs((slv->dc.t_delay_with_childs - slv->dc.t_delay_childs + t_diff) / 2);
            slv->dc.t_delay_parent_previous =
                times_parent[parent_previous_port] - times_parent[slv_parent->entry_port];
            if (slv->entry_port == 0) {
                slv->pdelay =
                    slv_parent->pdelay + slv->dc.t_delay_parent_previous + slv->dc.t_delay_slave;
            } else {
                slv->pdelay = slv_parent->pdelay + (abs((osal_int32_t)times_slave[slv->entry_port] -
                                                        (osal_int32_t)times_slave[0] + t_diff) /
                                                    2);
            }
        }

        ec_log(100, "DISTRIBUTED_CLOCK",
               "slave %2d: delay_childs %" PRIi32 ", delay_slave %" PRIi32
               ", delay_parent_previous_slaves %" PRIi32 ", delay_with_childs %" PRIi32 "\n",
               slave, slv->dc.t_delay_childs, slv->dc.t_delay_slave,
               slv->dc.t_delay_parent_previous, slv->dc.t_delay_with_childs);

        // write propagation delay
        ec_log(100, "DC_CONFIG", "slave %2d: sysdelay %" PRIi32 "\n", slave, slv->pdelay);
        check_ec_fpwr(pec, slv->fixed_address, EC_REG_DCSYSDELAY, &slv->pdelay, sizeof(slv->pdelay),
                      &wkc);
    }
