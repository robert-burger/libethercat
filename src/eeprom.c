...
#define SII_REG(ac, adr, val)                                                                \
    cnt = 100u;                                                                              \
    do {                                                                                     \
        ret = ec_fp##ac(pec, pec->slaves[slave].fixed_address, (adr), (osal_uint8_t*)&(val), \
                        sizeof(val), &wkc);                                                  \
    ...
    if (ret == EC_OK) {
        ...
        do {
            ...
            if (osal_timer_expired(&timeout) == OSAL_ERR_TIMEOUT) {
                ec_log(10, "EEPROM_TO_EC",
                       "slave %2d: failed setting eeprom to EtherCAT: eepcfgstat %04X, trying to "
                       "ignore ...\n",
                       slave, eepcfgstat);
                // ret = EC_ERROR_EEPROM_CONTROL_TO_EC;
                break;
...
int ec_eepromread(ec_t* pec, osal_uint16_t slave, osal_uint32_t eepadr, osal_uint32_t* data) {
    ...
    if (ret == EC_OK) {
        ret = ec_fpwr(pec, pec->slaves[slave].fixed_address, EC_REG_EEPADR, (osal_uint8_t*)&eepadr,
                      sizeof(eepadr), &wkc);
    ...
    if (ret == EC_OK) {
        eepcsr = 0x0100;
        ret = ec_fpwr(pec, pec->slaves[slave].fixed_address, EC_REG_EEPCTL, (osal_uint8_t*)&eepcsr,
                      sizeof(eepcsr), &wkc);
        if ((ret == EC_OK) && (wkc != 1u)) {
    ...
    if (ret == EC_OK) {
        *data = 0;
        ret = ec_fprd(pec, pec->slaves[slave].fixed_address, EC_REG_EEPDAT, (osal_uint8_t*)data,
                      sizeof(*data), &wkc);
        if ((ret == EC_OK) && (wkc != 1u)) {
...
int ec_eepromwrite(ec_t* pec, osal_uint16_t slave, osal_uint32_t eepadr, osal_uint16_t* data) {
        ...
        do {
            ...
            if (osal_timer_expired(&timeout) == OSAL_ERR_TIMEOUT) {
                ec_log(1, "EEPROM_WRITE",
                       "waiting for eeprom !busy failed, "
                       "wkc %d\n",
                       wkc);
    ...
    if (ret == EC_OK) {
        osal_timer_init(&timeout, 100000000);  // 100 ms
        do {
            ret = ec_fpwr(pec, pec->slaves[slave].fixed_address, EC_REG_EEPDAT, (osal_uint8_t*)data,
                          sizeof(*data), &wkc);
...
int ec_eepromread_len(ec_t* pec, osal_uint16_t slave, osal_uint32_t eepadr, osal_uint8_t* buf,
                      osal_size_t buflen) {
    assert(pec != NULL);
    assert(buf != NULL);
...
#define ADD_MBX_SUPP_TEXT(flag, text)                                                       \
    if (slv->eeprom.mbx_supported & (flag)) {                                               \
        if (mbx_supported_buf_pos == 0) {                                                   \
            mbx_supported_buf_pos += snprintf(&mbx_supported_buf[mbx_supported_buf_pos],    \
                                              128 - mbx_supported_buf_pos, "%s", (text));   \
        } else {                                                                            \
            mbx_supported_buf_pos += snprintf(&mbx_supported_buf[mbx_supported_buf_pos],    \
                                              128 - mbx_supported_buf_pos, ", %s", (text)); \
        }                                                                                   \
...
#undef ADD_MBX_SUPP_TEXT
        ...
        eeprom_log("slave %2d: Vendor 0x%08X, ProductCode 0x%08X, SupportedMbx %s\n", slave,
                   slv->eeprom.vendor_id, slv->eeprom.product_code, mbx_supported_buf);
            ...
            do {
                ...
                switch (cat_type) {
                    ...
                    case EC_EEPROM_CAT_STRINGS: {
                        ...
                        (void)ec_eepromread_len(pec, slave, cat_offset + 2, &ec_eeprom_buf[0],
                                                cat_len * 2u);
                        ...
                        if (slv->eeprom.strings_cnt > LEC_MAX_EEPROM_CAT_STRINGS) {
                            do_eeprom_log(10, "EEPROM_STRINGS",
                                          "        : warning: can only store %" PRIu64 " strings\n",
                        ...
                        for (i = 0; i < slv->eeprom.strings_cnt; ++i) {
                            ...
                            if (string_len > (LEC_MAX_STRING_LEN - 1u)) {
                                do_eeprom_log(10, "EEPROM_STRINGS",
                                              "        : warning string is %d bytes long, can only "
                                              "read %" PRIu64 " bytes.\n",
                            ...
                            if (i >= LEC_MAX_EEPROM_CAT_STRINGS) {
                                ...
                                (void)strncpy(&tmp[0], (osal_char_t*)&ec_eeprom_buf[local_offset],
                                              read_string_len);
                                local_offset += string_len;
                                tmp[read_string_len] = '\0';

                                do_eeprom_log(10, "EEPROM_STRINGS",
                                              "        (-)  string %2" PRIu32 ", length %2d : %s\n",
                                              i, string_len, tmp);
                            } else {
                                (void)strncpy(&slv->eeprom.strings[i][0],
                                              (osal_char_t*)&ec_eeprom_buf[local_offset],
                                              read_string_len);
                                local_offset += string_len;
                            ...
                            if (local_offset > (cat_len * 2u)) {
                                do_eeprom_log(
                                    5, "EEPROM_STRINGS",
                                    "          something wrong in eeprom string section\n");
                                break;
                            }
                        }
                    ...
                    case EC_EEPROM_CAT_GENERAL: {
                        ...
                        do_eeprom_log(10, "EEPROM_GENERAL",
                                      ...
                                      slv->eeprom.general.group_idx, slv->eeprom.general.img_idx,
                                      slv->eeprom.general.order_idx, slv->eeprom.general.name_idx);
                        break;
                    }
                    case EC_EEPROM_CAT_FMMU: {
                        ...
                        while (local_offset < (cat_offset + cat_len + 2u)) {
                            ...
                            while ((i < 4u) && (i < (cat_len * 2u))) {
                                if ((fmmu_idx < LEC_MAX_EEPROM_CAT_FMMU) &&
                                    (fmmu_idx < slv->fmmu_ch) && (tmp[i] >= 1u) && (tmp[i] <= 3u)) {
                                    slv->fmmu[fmmu_idx].type = tmp[i];
                                    slv->eeprom.fmmus[fmmu_idx].type = tmp[i];

                                    do_eeprom_log(10, "EEPROM_FMMU",
                                                  "          fmmu%" PRIu32 ", type %d\n", fmmu_idx,
                                                  tmp[i]);
                                }
                    ...
                    case EC_EEPROM_CAT_SM: {
                        do_eeprom_log(10, "EEPROM_SM", "slave %2d: entries %" PRIu32 "\n", slave,
                                      (osal_uint32_t)(cat_len / (sizeof(ec_eeprom_cat_sm_t) / 2u)));
                        ...
                        while (local_offset < (cat_offset + cat_len + 2u)) {
                            ...
                            if (slv->sm[j].adr == 0u) {
                                ...
                                do_eeprom_log(10, "EEPROM_SM",
                                              ...
                                              j, tmp_sms.adr, tmp_sms.len,
                                              (tmp_sms.activate << 16) | tmp_sms.ctrl_reg);
                    ...
                    case EC_EEPROM_CAT_TXPDO: {
                        ...
                        do {
                            ...
                            if (tmp_pdo.n_entry > 0u) {
                                (void)memset(&tmp_pdo.entries[0], 0,
                    ...
                    case EC_EEPROM_CAT_DC: {
                        ...
                        if (dcs_cnt > LEC_MAX_EEPROM_CAT_DC) {
                            ec_log(5, "EEPROM_DC",
                                   "slave %2d: can only store %" PRIu64
                                   " dc settings but got %d!\n",
                        ...
                        for (j = 0; j < dcs_cnt; ++j) {
                            ...
                            (void)ec_eepromread_len(pec, slave, local_offset,
                                                    (osal_uint8_t*)&tmp_dc, EC_EEPROM_CAT_DC_LEN);
                            local_offset += (osal_size_t)(EC_EEPROM_CAT_DC_LEN / 2u);

                            do_eeprom_log(10, "EEPROM_DC",
                                          "          cycle_time_0 %" PRIu32
                                          ", "
