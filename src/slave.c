    ...
    if (slv->sm_set_by_user != 0) {
...
#endif
        {
            ...
            for (osal_uint8_t sm_idx = 0; sm_idx < slv->sm_ch; ++sm_idx) {
                ...
                ec_log(100, "SLAVE_GENERATE_MAPPING_EEPROM", "slave %2d: txpdos %d, rxpdos %d, bitlen%d %" PRIu8 "\n",
                        slave, txpdos_cnt, rxpdos_cnt, sm_idx, bit_len);

                if (bit_len > 0u) {
                    ec_log(10, "SLAVE_GENERATE_MAPPING_EEPROM", "slave %2d: sm%d length bits %" PRIu8 ", bytes %" PRIu8 "\n",
